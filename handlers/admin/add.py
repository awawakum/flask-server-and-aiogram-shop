...
async def process_settings(message: Message):
    ...
    markup.add(InlineKeyboardButton(
        '+ Добавить товар', callback_data='add_category'))

    await message.answer('Настройка товаров:', reply_markup=markup)
...
async def category_callback_handler(query: CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await query.answer('Все добавленные типы товаров в эту категорию.')
...
async def add_category_callback_handler(query: CallbackQuery):
    await query.message.delete()
    await query.message.answer('Название товара?')
...
@dp.message_handler(IsAdmin(),  content_types=ContentType.TEXT, state=ProductState.cnt)
async def process_title(message: Message, state: FSMContext):

    async with state.proxy() as data:
        data['cnt'] = message.text
...
async def process_confirm(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        idx = md5(' '.join([title, body, price, cnt, tag]
                           ).encode('utf-8')).hexdigest()

        db.query('INSERT INTO products VALUES (?, ?, ?, ?, ?, ?, ?)',
                 (idx, title, body, image, int(price), int(cnt), tag))
...
@dp.callback_query_handler(IsAdmin(), product_cb.filter(action='minus'))
async def minus_product_callback_handler(query: CallbackQuery, callback_data: dict):

    product_idx = callback_data['id']
    db.query('UPDATE products SET cnt = cnt - 1 WHERE idx=?', (product_idx,))
    await query.answer('Количество уменьшено')

@dp.callback_query_handler(IsAdmin(), product_cb.filter(action='plus'))
async def plus_product_callback_handler(query: CallbackQuery, callback_data: dict):
...
async def show_products(m, products, category_idx):
    ...
    for idx, title, body, image, price, cnt, tag in products:
        ...
        markup.add(InlineKeyboardButton(
            '➖', callback_data=product_cb.new(id=idx, action='minus'),
        ),
            InlineKeyboardButton(
                '➕', callback_data=product_cb.new(id=idx, action='plus'),
            )
        )
        markup.add(InlineKeyboardButton(
